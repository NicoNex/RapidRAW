//! HTTP client for the external generative-inpaint backend, shared by both UIs.
//!
//! Talks to the RapidRaw AI-connector middleware (local server or cloud): uploads
//! the source image once (keyed by a content hash), then posts inpaint requests
//! with a base64 mask + prompt and composites the returned crop back to full res.
