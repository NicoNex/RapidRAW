...
//! dependency live here. `load_base_image_from_bytes`, `load_and_composite`, and
//! the cache helpers stay in src-tauri (they read AppSettings / caches). The
//! cancel-token plumbing is also dropped (core has no cancel infrastructure).
//!
//! `composite_patches_on_image` is here (pure: image + adjustments JSON -> image)
//! so both the Tauri and relm4 render paths bake AI patches onto the base image
//! before the adjustment pipeline runs.
...
use crate::mask_generation::{AiResolver, MaskDefinition, SubMask, generate_mask_bitmap};
...
/// Bake visible AI patches (`current_adjustments["aiPatches"]`) onto `base_image`.
/// Each patch's stored `patchData.color` is alpha-blended through its mask (the
/// stored `patchData.mask`, or regenerated from `sub_masks` if absent). `ai`
/// resolves AI sub-mask types when regenerating; pass `None` if not built.
pub fn composite_patches_on_image(
    ...
    for patch_obj in visible_patches {
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            generate_mask_bitmap(&mask_def, base_w, base_h, 1.0, (0.0, 0.0), None, ai)
