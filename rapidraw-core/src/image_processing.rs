...
// `non_raw_tonemapper` are the resolved tonemapper name strings (e.g. "agx"/"basic").
pub fn resolve_tonemapper_override(
    enabled: bool,
    is_raw: bool,
    raw_tonemapper: &str,
    non_raw_tonemapper: &str,
) -> Option<u32> {
    if !enabled {
        return None;
    }
    let tm = if is_raw { raw_tonemapper } else { non_raw_tonemapper };
...
// Coupling-cut: `calculate_auto_adjustments` was a #[tauri::command] reading
// AppState; the reusable analysis lives in `perform_auto_analysis` /
// `auto_results_to_json` which remain available to callers.
