...
/// Download/progress events surfaced to the host UI. The caller (src-tauri,
/// relm4) maps these onto its own event channel; core stays UI-agnostic.
/// Replaces the former `tauri::AppHandle::emit` seam.
pub enum DownloadEvent<'a> {
    /// A model download started (model display name).
    Start(&'a str),
    /// A model download finished (model display name).
    Finish(&'a str),
    /// Free-form progress message (e.g. "Denoising… 40%").
    Progress(&'a str),
}

/// Progress sink. Must be `Send + Sync` since it is called across `.await`
/// points in the async download/init paths.
pub type Progress<'a> = &'a (dyn Fn(DownloadEvent) + Send + Sync);
...
async fn download_and_verify_model(
    ...
    if !is_valid {
        ...
        progress(DownloadEvent::Start(model_name));
        download_model(url, &dest_path).await?;
        progress(DownloadEvent::Finish(model_name));
...
pub async fn get_or_init_clip_models(
    ...
    if !clip_tokenizer_path.exists() {
        progress(DownloadEvent::Start("CLIP Tokenizer"));
        download_model(CLIP_TOKENIZER_URL, &clip_tokenizer_path).await?;
        progress(DownloadEvent::Finish("CLIP Tokenizer"));
    }
...
fn run_native_denoise(
    ...
    for i in 0..total {
        ...
        if i % 10 == 0 {
            ...
            progress(DownloadEvent::Progress(&format!("Denoising… {:.0}%", pct)));
        }
...
pub fn run_ai_denoise(
    ...
    progress(DownloadEvent::Progress("Denoising (AI NIND)..."));
...
// --- AI sub-mask resolver -------------------------------------------------
//
// Render-time decoding of AI masks. Inference (run earlier by the generate_*
// commands) stores a base64 PNG in the sub-mask's parameters; here we decode it
// and map it back through the image's rotate/flip/crop/scale transform so the
// mask lines up with the displayed (possibly geometry-edited) photo. This is
// pure (no ort) but lives in the `ai` module because it reads the AI parameter
// structs above. Injected into core's mask generator via `AiResolver`.
...
fn generate_ai_bitmap_from_full_mask(full_mask_image: &GrayImage, tf: &TransformParams) -> GrayImage {
...
fn generate_ai_sky_bitmap(
    ...
    crate::mask_generation::apply_grow_and_feather(
        &mut mask,
        grow_feather.grow,
...
pub fn ai_sub_mask_resolver(
    sub: &crate::mask_generation::SubMask,
    width: u32,
    height: u32,
    scale: f32,
    crop_offset: (f32, f32),
